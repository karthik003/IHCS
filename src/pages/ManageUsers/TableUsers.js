...
const TableUsers = (props) => {
  ...
  function renderTableRow() {
    ...
    return displayTable.map((item) => (
      <tr
        key={item.id}
        onClick={() => setactions(item)}
        className={actions && actions.id === item.id ? "action-active" : ""}
      >
        {item.row.map((r, index) => {
          return (
            <td
              ...
              style={{ color: r.color }}
            >
              {r.icon ? <i className={r.icon}></i> : r.value}
            </td>
          );
        })}
      </tr>
  ...
  return (
    <div className="table-users">
      ...
      {actions && <ActionsPopup data={actions} setactions={setactions} seteditUser={seteditUser} />}
      <EditUserPopup user={actions} editUser={editUser} seteditUser={seteditUser} setactions={setactions} />
    </div>
  );
};
